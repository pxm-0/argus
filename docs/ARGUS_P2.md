# Argus P2 Specification

Status: draft for P2 execution
Target repo: `/srv/argus`
...
P2 turns Argus from an operational foundation into a safer daily-use platform.
...
P2 should still be conservative. It should not convert Argus into a public admin panel, general-purpose deployment system, or Kubernetes replacement.
...
```text
/srv/argus exists and is a Git repo.
...
Cloudflare hostname in P1: hello-nginx.argus.invalid
```
...
Workloads remain agnostic by default. Privacy and access are policy states controlled by Argus.
...
scripts/smoke-test
git status --short
systemctl is-active argus-control-api.service || true
systemctl is-active argus-metrics.timer || true
...
```text
/usr/local/bin/argus-logs                  -> /srv/argus/scripts/argus-logs
/usr/local/bin/argus-backup-plan           -> /srv/argus/scripts/argus-backup-plan
/usr/local/bin/argus-backup-run            -> /srv/argus/scripts/argus-backup-run
/usr/local/bin/argus-restore-plan          -> /srv/argus/scripts/argus-restore-plan
/usr/local/bin/argus-events                -> /srv/argus/scripts/argus-events
/usr/local/bin/argus-smoke-test      -> /srv/argus/scripts/smoke-test
/usr/local/bin/argus-access-preview        -> /srv/argus/scripts/argus-access-preview
/usr/local/bin/argus-access-apply          -> /srv/argus/scripts/argus-access-apply
/usr/local/bin/argus-cloudflare-plan       -> /srv/argus/scripts/argus-cloudflare-plan
```
...
PASS operator links do not point outside /srv/argus/scripts
```
...
```text
- discover /srv/argus/workloads/*/manifest.json
...
- token file remains /etc/argus/control-token
...
- refuse if destination is not under /srv/argus/runtime/backups/<id>
...
[ ] argus-backup-run hello-nginx creates timestamped artifact
...
```text
1. Run discovery.
2. Run argus-migrate-workload-plan <id>.
...
```text
scripts/argus-cloudflare-activate-preview
```
...
```text
scripts/argus-cloudflare-activate
```
...
```text
scripts/argus-cloudflare-rollback
```
...
git pull --ff-only
scripts/smoke-test
git tag -a p2-complete -m "Argus P2 complete"
...
```json
{
  ...
  "artifactPath": "/srv/argus/runtime/backups/hello-nginx/20260630-000000",
  "redacted": true
}
```
...
```text
/srv/argus/runtime/cloudflare-activation/<timestamp>.json
```
...
It should manage approved /usr/local/bin symlinks for Argus commands.
...
Create scripts/argus-dashboard-state to write ignored runtime/dashboard-state.json.
...
Run argus-backup-run hello-nginx.
...
Create scripts/argus-access-reconcile.
...
```text
Expand smoke-test and argus-doctor for P2 invariants.
...
- argus-control-api.service: active/inactive
- argus-metrics.timer: active/inactive
