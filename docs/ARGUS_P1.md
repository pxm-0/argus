# Argus P1 - Workload Operations and Organization

Status: Draft for implementation
Owner: Argus
Target system: `oreochiserver`
Depends on: Argus P0 complete
...
P0 established Argus as a private, Git-tracked personal ECS-style control plane.

P1 turns that foundation into a useful daily operations platform.

The main goal of P1 is to organize real workloads under the Argus structure, add safe operational commands, improve the dashboard for workload management, introduce backup/restore planning, and test Cloudflare protected access only on approved low-risk workloads.
...
P1 should not turn Argus into Kubernetes, a full CI/CD platform, or a public hosting system. It should make the current personal ECS box more organized, safer to operate, and easier to use from a phone or laptop.
...
- `argus-control-api.service` is active.
- `argus-metrics.timer` is active.
...
1. Move selected workloads into the canonical Argus workload layout.
...
1. Do not expose the Argus dashboard publicly.
...
Argus should become the canonical organizational home for workloads.
...
├── README.md
├── manifest.json
├── source/        # ignored by Argus Git by default
├── data/          # ignored
├── env/           # ignored
├── backups/       # ignored
└── runtime/       # ignored
...
Argus Git should track only safe metadata unless explicitly directed otherwise:
...
```bash
argus-logs <workload-id>
argus-action-preview restart <workload-id>
argus-action-apply restart <workload-id> --confirm <workload-id>
argus-backup-plan <workload-id>
argus-backup-run <workload-id>
argus-restore-plan <workload-id>
argus-workload-status <workload-id>
```
...
- Do not use the Argus dashboard as the first Cloudflare test.
...
As the operator, I want workloads organized under `/srv/argus/workloads` so that the server is understandable and maintainable.
...
- `argus-logs <id>` works for approved workloads.
...
- `argus-action-preview restart <id>` shows what would happen.
- `argus-action-apply restart <id> --confirm <id>` requires exact workload ID confirmation.
...
Workloads
      ...
      +-- canonical roots under /srv/argus/workloads/<id>
```
...
This means the operator requested Cloudflare protected access, but Argus has not successfully applied it yet.
...
```bash
argus-migrate-workload-plan <workload-id>
```
...
- First workload lives under `/srv/argus/workloads/<id>/source`.
- Old path is preserved or documented.
- Compose project name is unchanged.
- Health check passes.
- Dashboard shows migration status.
- No source/env/runtime/backups are tracked by Argus Git.
...
```bash
sudo ln -sf /srv/argus/scripts/argus-logs /usr/local/bin/argus-logs
sudo chmod +x /srv/argus/scripts/argus-logs
```
...
```bash
argus-logs <workload-id>
argus-logs <workload-id> --tail 100
argus-logs <workload-id> --follow
```
...
- `argus-logs <id>` works for approved workload.
- `argus-logs unknown` exits nonzero.
...
```bash
sudo ln -sf /srv/argus/scripts/argus-backup-plan /usr/local/bin/argus-backup-plan
sudo ln -sf /srv/argus/scripts/argus-backup-run /usr/local/bin/argus-backup-run
sudo ln -sf /srv/argus/scripts/argus-restore-plan /usr/local/bin/argus-restore-plan
sudo chmod +x /srv/argus/scripts/argus-backup-plan
sudo chmod +x /srv/argus/scripts/argus-backup-run
sudo chmod +x /srv/argus/scripts/argus-restore-plan
```
...
```json
{"timestamp":"2026-06-30T12:00:00Z","actor":"argus","action":"restart","workloadId":"demo-app","result":"success","details":"health check passed"}
```
...
- `argus-events` prints recent events.
...
```bash
git tag -a p1-complete -m "Argus P1 complete"
```
  ...
  "workloads": [
    {
      ...
      "kind": "web-app",
      "paths": {
        "root": "/srv/argus/workloads/demo-app",
        "source": "/srv/argus/workloads/demo-app/source",
        "manifest": "/srv/argus/workloads/demo-app/manifest.json",
...
```bash
argus-logs <workload-id> [--tail N] [--follow]
```

Safe logs via Compose.

### 13.3 `argus-action-preview restart`
...
### 13.4 `argus-action-apply restart`
...
- Token is stored in `/etc/argus/control-token`.
...
- git status
- argus-smoke-test
- argus-control-api.service status
- argus-metrics.timer status
- Caddy validation
- Tailscale Funnel status
...
Do not choose Intake OS, Uptime Kuma, PostgreSQL, Argus, or any sensitive workload as the first migration.
...
```text
Expand argus-smoke-test for P1.
...
P1 is complete when Argus becomes the safe daily operating layer for the server:
