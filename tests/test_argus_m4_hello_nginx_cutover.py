...
class CutoverTest(unittest.TestCase):
 def test_strips_loopback_port_from_minimal_target(self):
  source={'services':{'web':{'image':'nginx:stable','container_name':'hello-nginx','restart':'unless-stopped','ports':[{'host_ip':'127.0.0.1','target':80}]}}}
  self.assertEqual({'name':'hello-nginx','services':{'web':{'image':'nginx:stable','restart':'unless-stopped'}}},target_compose(source))
