...
class OptionalEvidenceCollectorTests(unittest.TestCase):
    ...
    def test_sources_have_independent_bounds_and_socket_bindings(self) -> None:
        ...
        self.assertEqual(3, len(registry.sources))
        self.assertEqual(3, len({source.transport["socketPath"] for source in registry.sources.values()}))
        self.assertEqual({"oreochiserver.configured-roots", "oreochiserver.process-listeners", "oreochiserver.proxy-overlay"}, set(registry.sources))
        self.assertEqual("filesystem:reviewed-root-summaries", registry.sources["oreochiserver.configured-roots"].allowlist["apis"][0])
        self.assertEqual(2, len(registry.sources["oreochiserver.process-listeners"].allowlist["commands"]))
